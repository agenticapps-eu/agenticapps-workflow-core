...
> **§13 was retired at spec 2.0.0, and the rows above still describe it.** That
> is deliberate. `codex-ts-declare-first`, `opencode-ts-declare-first` and
> `pi-ts-declare-first` were really shipped and really bound, and pi really did
> reach `full` by binding §13 — those are facts about what these repos contain,
> not claims about a live obligation, and deleting them would erase true
> implementation history.
>
> What changed is their **status**, not their truth: the `implements_spec`
> versions in this table are all 1.x, and spec 2.0.0 obsoletes every prior-major
> claim under §09. Each host must re-assert against 2.0.0, at which point §13 is
> simply not among the sections it can cite. No host is non-conformant for having
> implemented a section that later retired.
