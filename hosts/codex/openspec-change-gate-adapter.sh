...
#
# Carried into core from the archived codex-workflow checkout, and CORRECTED on
# the way: the sentence here used to say the gate's verdict is "validate-green
# AND REVIEWS.md >= 2 reviewers". That stopped being true at gate 2.0.0.
#
# The gate blocks on ONE condition: `openspec validate --all` is not green.
# Review evidence is computed and REPORTED — count, verdicts, independence all
# produce NOTE lines and none of them fails any surface. Two rejections open the
# gate exactly as two approvals do.
#
# This is still a THIN ADAPTER: all verdict logic lives in
# openspec-change-gate.sh and must never be re-implemented here. What this file
# owns is the codex payload shape and the codex stdout contract, below.
#
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# This host's identity, so the gate's independence REPORTING does not count
# codex's own reviews as independent. Set here rather than inherited. Note what
# this does and does not do: it changes a NOTE line, not a verdict — nothing
# about reviewer independence blocks anything. It is still worth setting,
# because a report that counts self-review is a report that misleads.
