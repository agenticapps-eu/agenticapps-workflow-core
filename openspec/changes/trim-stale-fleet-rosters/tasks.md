...
- [ ] 0.1 Land the `vestigial-surface-removal` delta: the record/instrument
...
- [ ] 0.2 For each artifact §4 deletes, record the three conditions **as
...
- [ ] 1.1 Remove `agents-task-viewer` from
      ...
      than today.
- [ ] 1.2 Run `tools/check-shims.sh` and confirm no `MISSING REPO` line. Confirm
...
- [ ] 1.3 Run `tools/check-shims.test.sh`. It builds synthetic FLEET fixtures and
...
- [ ] 2.1 **RED first.** Add to `tools/conformance-harness-reporting.test.sh` an
...
- [ ] 2.2 Trim `ROSTER` in `tools/change-gate-conformance.sh` (lines 1073–1076)
...
- [ ] 2.3 Observe 2.1 GREEN. Run both harnesses with `--family` and record the
      coverage line each now prints; `roster_total` must read 2, not 6.
- [ ] 2.4 Correct what the sweep claims, in both harnesses' own comments: two
...
- [ ] 3.1 **RED first, and it must be a real one.** Assert that
...
- [ ] 3.2 Remove from `tools/change-gate-conformance.sh`: the `--resolve` flag,
...
- [ ] 3.3 Remove the same probe and reason string from
      `tools/reviewer-cli-conformance.sh`, which implements the reporting half
      only.
- [ ] 3.4 Delete assertion J3 from `tools/conformance-harness-reporting.test.sh`.
...
- [ ] 3.5 Retire `reference-implementations/shared-install/resolve-core-artifact.sh`.
...
- [ ] 3.6 Retire `tools/resolve-core-artifact-conformance.sh` under §0, and
...
- [ ] 3.7 Check whether any requirement in `conformance-harness-reporting` names
      ...
      checked.
- [ ] 3.8 Observe 3.1 GREEN. `shellcheck -S warning` clean on both harnesses.

## 4. `drift-report.sh` is retired

- [ ] 4.1 Record its final measurement in `CHANGELOG.md` before deleting it —
      `OK: 0 · DRIFT: 0 · SKIP: 60` on 2026-08-12 — because after the deletion
      nothing can produce that number again.
- [ ] 4.2 Delete `tools/drift-report.sh` and `tools/drift-report.test.sh` under
...
- [ ] 4.3 Confirm by sweep that nothing invokes either: `install.sh`, the git
...
- [ ] 5.1 `spec/09-conformance.md` — the `## Drift` section describes
...
- [ ] 5.2 `spec/20-conformance-harness-reporting.md` — remove the pin-and-resolve
      ...
      them is wrong.
- [ ] 5.3 `GATE-INVENTORY.md:67` — the row reads `migration framework +
      ...
      about each item.
- [ ] 5.4 `README.md` — `:103` and `:137` present `drift-report.sh` as the
...
- [ ] 5.5 `reference-implementations/README.md` — `:61–64` are the per-host
...
- [ ] 6.2 Step 4 code review on the diff by a reviewer of another vendor.
- [ ] 6.3 `CHANGELOG.md` entry covering the four rosters, three retirements, the
      removed requirement and the amended one.
- [ ] 6.4 Full suite: `tools/conformance-harness-reporting.test.sh`,
