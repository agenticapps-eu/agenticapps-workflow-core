...
- [x] 8.4 Point the Claude installer at core rather than its vendored 1.0.0 copy, so core is the operational source of truth. **DONE 2026-07-31 — claude-workflow#109, ADR-0047.** `install.sh` and migration 0032 resolve the gate, the producer and the wrapper from core at a pinned commit, verify the sha256, and publish the verified bytes; the three vendored copies are deleted. What the task said (the producer) turned out to be one of three artifacts stale at the same time — gate 1.3.1 vs 2.0.0, producer 1.0.0 vs 1.2.0, wrapper 1.1.0 vs 1.2.0 — so all three moved together rather than leaving one installer half-pinned.
  - **The mechanism existed and was wired to nothing.** `resolve-core-artifact.sh` and `core-vendor.manifest` were written on 2026-07-28 to end per-host vendoring, and in claude-workflow they were **untracked files** — never committed, read by no code. Task 8.3 added the producer's path mapping to a resolver that no caller invoked. A capability nobody calls is indistinguishable from one that does not exist, which is the same shape as defect 3 of this change (a normative `SHALL` whose gate contained no reference to it).
  - **Re-vendoring was rejected.** It fixes the bytes and keeps the mechanism that made them wrong. `codex-workflow`'s manifest is a provenance record for copies, not an install-time pin; claude-workflow is the first host where the pin actually decides what gets published.
  - **Fails closed.** An installer that cannot verify what it is about to write into `~/.agenticapps/bin` stops. A fallback to the copy on disk, run on 2026-07-31, would have republished gate 1.3.1 over the 2.0.0 this change had just shipped.
  - **Resolving from the pin immediately falsified three assumptions the vendored copy had been hiding**, all in the migration suite's producer test: the wrapper delivers the prompt on stdin, not argv; `AGENT_SELF=none` is rejected by 1.2.0; a review with no verdict line is not counted. That test had been green against a copy nothing ships. Suite 218/4 → 226/0.
  - **Known limit, recorded not glossed:** the pin names `2b82a91`, the tip of core's `feat/step3-hook-shims-and-dead-gate-removal` (PR #47), not a commit on main. If #47 is squash-merged the sha is orphaned and the pin must be advanced. The failure is loud — an unresolvable pin is a failing row and a refusing installer, never a silent fallback to a stale copy.
...
- [x] 8b.6 **PUBLISHED 2026-07-31: gate 2.0.0 and producer 1.2.0 are live in `~/.agenticapps/bin/`.** Both byte-identical to the tracked sources; the published gate scores 71/71 on its own conformance harness. Verified against the exact fleet condition — a pre-trailer `REVIEWS.md` under an active change, driven through hook mode: gate 1.4.0 returned exit 2 `BLOCKED`, gate 2.0.0 returns exit 0 with a NOTE saying the file predates the trailer format. The 2026-07-30 outage scenario, re-run, is now benign. **No fleet event.** Superseded by gate 2.0.0: reviews do not block, so publishing cannot block anyone. The precondition this task carried (clear or announce the fleet's review state first) existed only because an unreviewed change was refused a code edit. Under 2.0.0 those repos get a NOTE telling them a plan review is worth running, and carry on working. 1.5.0's early publication on 2026-07-30 remains the reason 1.6.0 was never published and 2.0.0 supersedes both; see `~/.agenticapps/install.log` for the rollback record.
...
- [x] 9b.18 Bump to gate **1.6.0** (was 1.5.0), run `tools/change-gate-conformance.sh` green — its `TOTAL:` line must report zero failed and zero inconclusive — and publish. **Bumped and green at 69/69; NOT published**, which is the remaining half. 1.5.0 is not amended in place because it briefly reached the fleet on 2026-07-30, and a version that ever shipped must keep meaning one thing. Publication is still gated on 8b.7
  **SUPERSEDED 2026-08-01 by gate 2.0.0 — 1.6.0 was never published and never will be.** Ticked on the same basis as 8b.7: the task is discharged, not performed. Its precondition (8b.7) dissolved, and by the time it did, the tracked gate had moved past 1.6.0 — publishing it now would republish a superseded version over a live one, which is the downgrade `install-shared-artifact.sh` exists to refuse. What 9b.18 was actually for — the round-9b hardening reaching the fleet in a version that passes its own harness — is satisfied: gate **2.0.0** is live in `~/.agenticapps/bin/`, byte-identical to `reference-implementations/openspec-change-gate/openspec-change-gate.sh` (sha256 `4ad996cb…76780`, both sides verified 2026-08-01), and scores **71/71 passed, 0 failed, 0 inconclusive**.
  Every 9b behaviour survives the withdrawal, as reporting rather than blocking — the harness still carries a row for each: the verdict-and-substance predicate (`verdicts with no body count zero`), conflicting verdicts (`a section with conflicting verdicts does not count`), trailer identity (`no trailer` / `two trailers` / `a non-final trailer` each count zero), digest validation (`a malformed digest VALUE counts zero`), emphasis normalisation (`bolded` and `underscore-emphasised verdict counts`), and the reviewer-name bypass (`an unrecognised reviewer name does not count`). 2.0.0 changed what the gate *does* with the count, not how it derives it. 1.6.0 is therefore dead text, not deferred work
...
- [x] 10.2 Run `openspec-change-gate.sh --ci` green. **Verified 2026-08-01 on `main`: exit 0, `OK — all active changes validate.`** It emits two NOTEs and blocks on neither, which is 2.0.0 behaving as §18 now describes: `shim-project-hooks` has 2 reviewers of whom gemini and codex requested changes (allowed on quorum, objection restated), and this change's own review no longer matches its text since the artifacts moved under it (reported stale, not blocking). Both are the reported-not-enforced path — a `--ci` run that is green *because* nothing is enforced is only meaningful alongside 71/71 on the conformance harness, which is where the reporting itself is tested
