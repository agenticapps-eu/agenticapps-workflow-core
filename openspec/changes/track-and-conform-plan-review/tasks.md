...
- [x] 8b.4 Re-review every other active change the inventory found, or record explicitly which are accepted as blocked and why. **INVENTORY TAKEN 2026-07-30 — 37 active changes across 6 repos, none carrying a trailer:** `fx-signal-agent` 10, `fbc-platform` 8, `agenticapps-dashboard-add-agent-board` 8, `agenticapps-dashboard` 5, `callbot` 3, `agenticapps-roadmap` 1. This wave is the gate's precondition, not its follow-up.
  **DISCHARGED 2026-07-31 by the second branch — recorded as accepted-blocked, not re-reviewed** (operator's decision). All 37 are accepted as blocked by gate 1.5.0 on publication. The rationale, and the limits of it:
  ...
  - **This is a real cost being accepted, not a cost being denied.** Six repos will each hit a hard block at their next code edit. Nobody has been told. That notice is the residual obligation and it is task 8b.7, not a footnote here.
- [ ] 8b.6 **Re-publish gate 1.5.0 only after 8b.4 and 8b.7 complete.** 8b.4 is now discharged by acceptance rather than by clearing the fleet, which *moves* the precondition to 8b.7 (announce the block) — it does not remove it. Publishing with 8b.7 open would ship the same unannounced six-repo outage as 2026-07-30, this time deliberately. It was published early on 2026-07-30, blocking all six repos, and was rolled back with `--allow-downgrade openspec-change-gate.sh` (see `~/.agenticapps/install.log`). The tracked 1.5.0 is unaffected; re-publishing is one command once the fleet carries trailers
...
- [ ] 8b.7 **Announce the block before publishing, since 8b.4 accepted it rather than clearing it.** Six repos will each hit a hard `PreToolUse` block at their next code edit with no warning. Write the one-screen notice — what blocks, the single producer command that clears it, and `GSD_SKIP_REVIEWS=1` as the documented escape hatch — and put it where each repo's next operator will actually read it. Publishing without this repeats 2026-07-30 with the surprise merely rescheduled, which is the one lesson that round was supposed to have bought
