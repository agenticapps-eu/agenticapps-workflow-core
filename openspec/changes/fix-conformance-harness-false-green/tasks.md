...
- [ ] 1.1 `tdd="true"` — write `tools/conformance-harness-reporting.test.sh`,
      ...
      artifacts)
- [ ] 1.2 Row: a named target that does not exist → non-zero, in both shapes
- [ ] 1.3 Row: a named target that is zero-byte → non-zero, and NOT scored as
      rows. This is the false-green condition: an empty script exits 0 and
      passes every `expect 0` row
- [ ] 1.4 Row: a named target that is a directory → unscoreable. `[ -s ]` alone
      accepts one
- [ ] 1.5 Row: a named target that is non-empty but unreadable → reported
...
- [ ] 1.6 Row: three named targets to a multi-target harness, middle one absent
      → first and third scored in full, exit non-zero. Guards against fixing
      1.2 with an `exit 2` abort
- [ ] 1.7 Row: a single-target harness aborts non-zero naming the target and
      reason, and is NOT required to grow a tally
- [ ] 1.8 Row: a run whose rows are all inconclusive → non-zero. Scored total is
      passed + failed, so this trips the backstop
- [ ] 1.9 Row: a run that scored zero rows via a route other than 1.2–1.5 →
      non-zero, so the backstop is tested independently of the per-site fixes
- [ ] 1.10 Row: an unscoreable target and a row-failure in one run are reported
      distinguishably, with the unscoreable one naming which of the three
      conditions held
- [ ] 1.11 Row: roster mode prints `scored N of M` on a **complete** sweep, not
      only a narrowed one
- [ ] 1.12 Row: roster mode with an entry absent → names it and the reason,
      exits on the merits of the rest
- [ ] 1.13 Row: roster mode with an entry present but zero-byte → reported not
      scored with emptiness as the reason, does not by itself fail the run
- [ ] 1.14 Row: roster labels are stable logical names, containing no `$HOME`
      and no absolute workspace path
- [ ] 1.15 Row: whole roster absent → non-zero
- [ ] 1.16 Row: overlapping unscoreable conditions (a broken symlink is
      non-regular, empty and unreadable at once) report the first reason in the
      order regular / empty / readable
- [ ] 1.18 Row: roster mode + an explicit path in one invocation → usage error,
      scores nothing. Currently the explicit path is silently discarded
- [ ] 1.19 Row: whole roster absent → coverage line `scored 0 of M` IS printed
...
- [ ] 1.20 Row: a roster entry counts toward the numerator only if it
      contributed ≥1 scored row — otherwise `scored 6 of 6` is reachable with a
      scored total of zero
- [ ] 1.21 Row: no absolute path appears anywhere in roster-mode output,
      including the per-entry `═══` heading (`change-gate-conformance.sh:306`)
- [ ] 1.22 Row: a target path containing a newline or control character cannot
      forge a line resembling a PASS
- [ ] 1.23 Row: default `--family` reports a pin-and-resolve host as
      *resolvable, not attempted*, distinguishably from *not found*
- [ ] 1.24 Row: `--family --resolve` scores a resolvable entry, and reports a
...
- [ ] 1.25 **Observe RED.** Run against all five harnesses unmodified; record
...
- [ ] 2.1 `tdd="true"` — `change-gate-conformance.sh`: move the absence check
      off the entry-point loop into `score_gate`, counted as a failure, per
      `reviewer-cli-conformance.sh:169`
- [ ] 2.2 `tdd="true"` — `run-plan-review-conformance.sh`: same fix, same line,
      same shape
- [ ] 2.3 `tdd="true"` — replace every `[ -f ]` target check with independent
      regular / non-empty / readable tests that report which condition held
- [ ] 2.4 `tdd="true"` — coverage line in the two roster builders
      ...
      Logical labels only
- [ ] 2.5 `tdd="true"` — move the roster's absence filter AFTER the argument
...
- [ ] 2.6 `tdd="true"` — logical labels across ALL roster output, including the
      per-entry `═══ $GATE` heading at `change-gate-conformance.sh:306`. The
      coverage line alone leaves `$HOME` in the log on every run
- [ ] 2.7 `tdd="true"` — sanitize control characters and newlines from every
      echoed target path, on both the roster and explicit-path sides
- [ ] 2.8 `tdd="true"` — reject roster flag + explicit paths with a usage error
      instead of discarding the paths
- [ ] 2.9 `tdd="true"` — `--resolve`: opt-in, default off. Default mode reports a
...
- [ ] 2.10 `reviewer-cli-conformance.sh`: its explicit-path site MUST NOT change
      — it is the reference the others were fixed against. Roster builder only

## 3. Fix the single-target harnesses

- [ ] 3.1 `resolve-core-artifact-conformance.sh` and `shared-install-conformance.sh`:
...
- [ ] 3.2 Confirm no tally is introduced. The delta explicitly permits a
      single-target harness to satisfy the backstop by aborting before any row

## 4. The backstop

- [ ] 4.1 `tdd="true"` — all five: a run terminating with a scored total of zero
      exits non-zero and says on stderr that it certified nothing
- [ ] 4.2 Assert scored = passed + failed in the two harnesses tracking
      `inconclusive`, so an all-inconclusive run trips it
- [ ] 4.3 Fold the backstop INTO the existing `[ "$fail" -eq 0 ]` terminal
...
- [ ] 4.4 Build the all-inconclusive test seam. `run-plan-review-conformance.sh:256`
...
- [ ] 4.5 **Observe GREEN.** Re-run §1 against all five; every row passes

## 5. Close the spec gap

- [ ] 5.1 Add the harness-reporting requirement as a new spec section typed
...
- [ ] 5.2 Delta §00's framing sentence ("the requirements live in the
...
- [ ] 5.7 ADR under `adrs/`: three behaviours across five harnesses, why named
...
- [ ] 6.1 Run `--family` for real. Assert it reports `scored 4 of 6` naming
      `claude-workflow` and `codex-workflow` — the live case that motivated the
      change
- [ ] 6.2 Confirm the four present gates score exactly as before, row for row.
      This change must not move a single conformance verdict; if it does,
      something other than reporting changed
- [ ] 6.3 Run every harness against its real targets; confirm nothing
