...
- [x] 7.3 Record the installer prerequisite requirement (detect missing prerequisites, offer to install, user accepts first) in the Part-2 installer change so it is not lost with this one — **Part 2a opened as `installer-prerequisite-consent` (PR #80).** Investigating whether Part 2 was needed inverted its premise: four host installers already exist, and two of them (`codex-workflow`, `opencode-workflow`) run `npm i -g @fission-ai/openspec` with no consent, while the other two only instruct. So the requirement is not a clause of an installer yet to be built — it is a live defect in installers that ship today. The curl-able installer originally framed as Part 2 is deferred to Part 2b and gated on `PLAN-lightweight-fleet` step 4, since a fifth installer is the construction that doc says to defer
- [x] 7.4 Open the PR and archive the change — **PR #79 opened.** Archive is deliberately not run: `openspec archive` folds the delta into `openspec/specs/`, and doing that before the PR merges would land the fold and the proposal in one commit and lose the reviewable boundary. Archive after merge
